...
{
    public partial class wIngresoMercaderiasRE : Heredados.MiForm8
    {
        public wIngresoMercaderiasRE()
        {
            InitializeComponent();
        }
        ...
        public RetiquetadoEN eRetEN = new RetiquetadoEN();        
        ...
        {
            this.txtCorEnc.Text = pObj.CorrelativoRetiquetado;
            this.txtCodAlm.Text = pObj.CodigoAlmacenRE;
        ...
        public MovimientoCabeEN ObtenerMovimientoCabeIngresoBD(RetiquetadoEN pObj)
        {
            //asignar parametros
        ...
        public void ActualizareOperacion(RetiquetadoEN pObj)
        {
            ...
            {
                if (pObj.CEstadoRetiquetado == "0")//pendiente
                {
        ...
        public void VentanaAdicionar(RetiquetadoEN pObj)
        {
            ...
            RetiquetadoRN.ModificarCostosFaseEmpaquetado(pObj);
            RetiquetadoRN.ModificarCostosManoObraProTer(pObj);
            RetiquetadoRN.ModificarCostosIngresoIndirectoProDet(pObj);
            this.ActualizarRetiquetadoDeBaseDatos(pObj);
        ...
        public void LLenarMovimientoDetaDeBaseDatos(RetiquetadoEN pObj)
        {
        ...
        public void LLenarMovimientoDetaAlAdicionar(RetiquetadoEN pObj)
        {
            //asignar parametros
            RetiquetadoEN iRetEN = pObj;
            MovimientoCabeEN iMovCabEN = this.NuevoMovimientoCabeDeVentana();

            //ejecutar metodo
            this.eLisMovDet = MovimientoDetaRN.ListarMovimientoDetaParaIngresoMercaderiaRE(iRetEN, iMovCabEN);
        }

        public void ActualizarRetiquetadoDeBaseDatos(RetiquetadoEN pObj)
        {
            this.eRetEN = RetiquetadoRN.BuscarRetiquetadoXClave(pObj);
        }
        ...
        {
            //pasar la claveIngreso
            this.eRetEN.ClaveIngresoProductoTerminado = this.ObtenerClaveMovimientoCabe();

            //ejecutar metodo
            RetiquetadoRN.ModificarRetiquetado(this.eRetEN);            
        }
        ...
        public void AdicionarLotes()
        {
            //variables
            string iClaveMovimientoCabe = this.ObtenerClaveMovimientoCabe();
            List<LoteDetaEN> iLisLotDet = LoteDetaRN.ListarLotesDetaDeClaveMovimientoCabe(this.eRetEN.ClaveSalidaUnidadesEmpacar);
            
            //obtener el ultimo codigo lote de bd para este ProTer
            string iNuevoCodigoLote = LoteRN.ObtenerMaximoValorEnColumna(this.eRetEN.CodigoAlmacenRE, this.eRetEN.CodigoExistenciaRE);

            //lista a grabar
            List<LoteEN> iLisLotAdi = new List<LoteEN>();

            //recorrer cada loteDeta
            foreach (LoteDetaEN xLotDet in iLisLotDet)
            {
                ...
                iLotEN.CodigoAlmacen = this.eRetEN.CodigoAlmacenRE;
                iLotEN.CodigoExistencia = this.eRetEN.CodigoExistenciaRE;
                iLotEN.DescripcionExistencia = this.eRetEN.DescripcionExistenciaRE;
                iLotEN.CodigoLote = Numero.IncrementarCorrelativoNumerico(ref iNuevoCodigoLote, 6);
                iLotEN.FechaProduccionLote = xLotDet.FechaProduccionLote;
        ...
        {
            wDetalleIngresoMercaderiaRE win = new wDetalleIngresoMercaderiaRE();
            win.wIngMerRE = this;
        ...
        private void wIngresoMercaderiasRE_FormClosing(object sender, FormClosingEventArgs e)
        {
